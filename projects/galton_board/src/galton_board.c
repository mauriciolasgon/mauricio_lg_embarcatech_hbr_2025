...
void move_bola_y(bola *b, uint8_t *ssd, struct render_area *area); // Função que movimenta a bolinha no display de acordo com a velocidade
...
void move_bola_x(bola *b, uint8_t *ssd ,int direction)
{
    ...
    ssd[b->posicao[0] + (b->posicao[2]/8) * ssd1306_width]=mask0 & ~ b->face; // Apaga apenas a bolinha na posição anterior
...
void move_bola_y(bola *b, uint8_t *ssd, struct render_area *area)
{
    static int posicao=0;
    
    if(colisao(b, ssd)){ // Verifica se a bolinha colidiu com os pinos
...
int main()
{
    ...
    bola b1;
    b1.v = 2;
    b1.posicao[0] = ssd1306_width/2  - 1;
    b1.posicao[1] = b1.posicao[0]+1;
    b1.posicao[2] = 0;
    b1.face = 0x03; //preenchimento dos dois primeiros bits de cada coluna
